...
Caine.prototype.asyncCollect = function asyncCollect(mediator, cb) {
  var last = this.perPage;
  var page = 1;
  var out = [];
  var self = this;

  async.whilst(function() {
    return last === self.perPage;
  }, function(cb) {
...
Caine.prototype.handleIssue = function handleIssue(issue, cb) {
  ...
  if (pre.ok)
    return this.tagIssue(issue, null, pre, cb);
  ...
  this.asyncCollect(function(page, cb) {
    ...
    for (var i = comments.length - 1; i >= 0; i--)
      if (comments[i].user.login === self.user)
        break;
    var authorComments = comments.slice(i);

    // Filter author's comments
    authorComments = authorComments.filter(function(comment) {
    ...
    if (check.ok)
      return self.tagIssue(issue, comments, check, cb);
...
Caine.prototype.tagIssue = function tagIssue(issue, comments, check, cb) {
  var self = this;
  ...
  function onLabels(err) {
    ...
    self.cleanupComments(issue, comments, onCleanup);
  }
...
Caine.prototype.cleanupComments = function cleanupComments(issue,
                                                           comments,
                                                           cb) {
  var self = this;

  // Already loaded comments for the issue, delete them
  if (comments) {
    onComments(null, comments);
  } else {
    this.asyncCollect(function(page, cb) {
      var params = {
        user: self.repo.user,
