...
Contributing.prototype.splitMention = function splitMention(md, user) {
  var depth = 0;
  for (var i = 0; i < md.length; i++) {
    var item = md[i];

    if (/_start$/.test(item.type))
      depth++;
    else if (/_end$/.test(item.type))
      depth--;

    if (depth !== 0 || item.type !== 'paragraph')
      continue;

    var index = -1;
    var index = item.text.indexOf('@' + user);
    if (index === -1)
      continue;

    // @caineio_
    if (!/^(\W|$)/.test(item.text[index + 1 + user.length]))
      continue;
...
Contributing.prototype.test = function test(questions, answers, options) {
  ...
  // Find first mention
  if (options.user) {
    md = this.splitMention(md, options.user);

    if (!md)
      return { ok: false, mention: false, results: [] };
  }
  ...
  return { ok: ok, mention: true, results: results };
};
